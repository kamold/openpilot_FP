...
#define MSG_BREMSE_8    0x1AC   // TX by OP, 7N0 Spoof (MK60EC1 BS)


// Transmit of GRA_Neu is allowed on bus 0 and 2 to keep compatibility with gateway and camera integration
const CanMsg VOLKSWAGEN_PQ_TX_MSGS[] = {{MSG_HCA_1, 0, 5}, {MSG_GRA_NEU, 0, 4}, {MSG_GRA_NEU, 2, 4}, {MSG_LDW_1, 0, 8}, {MSG_MOB_1, 1, 6}, {MSG_GAS_1, 2, 6}, {MSG_AWV_1, 0, 8}, {MSG_BREMSE_8, 2, 8}};
  ...
  if (!relay_malfunction) {
    switch (bus_num) {
      case 0:
        ...
        break;
      case 2:
        if ((addr == volkswagen_torque_msg) || (addr == volkswagen_lane_msg) || (addr == MSG_AWV_1)) {
          // OP takes control of the Heading Control Assist and Lane Departure Warning messages from the camera
          bus_fwd = -1;
        } else {
